...
- **2026-01-14** — **Problem:** Firebase error "Write stream exhausted maximum allowed queued writes" continuing to appear after initial batch fixes. Console showed "Cache updated (134 sessions)", "Cache updated (135 sessions)", "Cache updated (136 sessions)" in rapid succession even when exercises were logged 7 minutes apart, followed by rate-limiting error. **Root cause:** The `onSnapshot` listener for session history was firing **multiple times for each single document write**. Firestore triggers `onSnapshot` multiple times during document creation due to: initial write, index updates, server timestamp resolution, and composite index updates. Each trigger caused the listener to: read all 135+ sessions, merge with local data, write to localStorage, and re-render UI. This created excessive Firestore operations exhausting the write stream quota. The error appeared on both phone PWA and laptop console because they share the same Firebase project and both have active listeners. **What I did:** Modified `onSnapshot` handler (lines 2595-2609) to check `snapshot.docChanges()` before processing. If `docChanges().length === 0`, the snapshot event is skipped with log "Skipping redundant snapshot event (no changes)". Initial load processes immediately for fast startup. Added JSDoc comment explaining that Firestore fires multiple events per document write due to indexing and timestamps. **Fix applied:** Session history listener now skips redundant snapshot events where no actual document changes occurred. Each exercise log now triggers only 1 processing cycle instead of 3, eliminating the "rapid succession" cache updates and preventing Firebase rate-limiting errors during workout logging (`pt/pt_tracker.html`).
