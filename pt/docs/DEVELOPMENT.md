...
- **2026-01-13** — **Problem:** Changes saved in pt_report.html (dosage, archiving, exercise edits) were not appearing in pt_tracker.html despite pt_report saving pt_modifications to Firestore runtime. **Root causes:** (1) pt_tracker loaded exerciseLibrary from Firestore but never applied pt_modifications, then updated its timestamp so subsequent checks skipped modifications entirely. (2) pt_tracker built runtime snapshots with exerciseLibrary from localStorage and synced back to Firestore with merge:true, overwriting pt_report's saved exerciseLibrary that included applied modifications. This violated the architectural principle that Firestore (with IndexedDB offline persistence) should be authoritative, not localStorage. **What I did:** (1) Added `applyModificationsAutomatically()` function to apply pt_modifications from Firestore runtime (dosage changes, archived exercises, edited exercises, role updates). Modified `loadRuntimeFromFirestore()` to call this function and clear pt_modifications after applying. (2) Removed exerciseLibrary from `buildRuntimeSnapshot()` so pt_tracker no longer overwrites Firestore runtime with localStorage data. pt_report is now the sole authoritative editor for exerciseLibrary in user runtime; pt_tracker only reads from Firestore (never writes to runtime). This ensures Firestore with IndexedDB is the source of truth. Note: pt_tracker still has dosage editing UI but changes may not persist correctly until full Phase 1 Firestore-first migration is complete (`pt/pt_tracker.html`).
