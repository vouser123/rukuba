...
- [ ] DN-031 | status:open | priority:P2 | risk:low | tags:[ui,docs,notifications] | file:pt-rebuild/public/index.html | issue:Phase 2 audio consistency pass: normalize spoken/beep behavior across counter, hold, and duration flows using SOUND_TRIGGER_AUDIT as baseline.
  - Context: Phase 1 inventory is complete in `pt-rebuild/docs/SOUND_TRIGGER_AUDIT.md` (direct callsites + UI trigger chains). Next phase should convert inconsistencies into explicit product rules and implementation tasks.
  - Options: (A) Produce a machine-readable trigger spec (JSON) and implement to that contract. (B) Keep runtime behavior but add feature flags/threshold constants and centralize trigger policy in one module. (C) Start with UX policy doc only, then apply code changes in a follow-up.
  - Constraints/Caveats: Must preserve existing safety/therapy workflows while reducing surprise; avoid regressions in short timers, side-switch announcements, and historical comparison cues.
