...
## Open Items
- [ ] DN-033 | status:in_progress | priority:P2 | risk:medium | tags:[migration,ui] | file:pt-rebuild/package.json, pt-rebuild/next.config.mjs, pt-rebuild/pages/rehab.js, pt-rebuild/pages/_app.js, pt-rebuild/lib/rehab-coverage.js, pt-rebuild/lib/supabase.js, pt-rebuild/hooks/useAuth.js, pt-rebuild/components/NavMenu.js, pt-rebuild/components/AuthForm.js, pt-rebuild/public/css/rehab-coverage.css | issue:Strangler Fig migration to Next.js (Pages Router). Existing static HTML pages remain untouched; new pages built in Next.js one at a time. Phase 1: scaffolding + shared architecture (lib/supabase.js, hooks/useAuth.js, components/NavMenu.js, components/AuthForm.js) + rehab_coverage migrated to pages/rehab.js.
  - Context: Phase 1 deliverables complete on nextjs branch. Shared architecture built: lib/supabase.js (single Supabase client), hooks/useAuth.js (session/signIn/signOut), components/NavMenu.js (replaces window.HamburgerMenu), components/AuthForm.js (shared sign-in form), lib/rehab-coverage.js (pure coverage calc functions extracted from rehab_coverage.html), pages/rehab.js (full React page using shared pieces), public/css/rehab-coverage.css (CSS extracted from rehab_coverage.html). pages/rehab.js builds and deploys via next build — confirmed by _next/ folder in Vercel Output. /rehab live on preview URL and shows auth form. Full verification pending (sign-in, coverage matrix, NavMenu, navigation between pages, console errors). Vercel fix required: framework preset was 'Other' → manually changed to 'Next.js' in dashboard (Settings → Build and Deployment). next.config.mjs uses .mjs extension to avoid type:module/CommonJS conflict. SUPABASE_URL and SUPABASE_ANON_KEY forwarded as NEXT_PUBLIC_* — no new Vercel vars needed. NEXTJS_MIGRATION.md synced to main branch as migration reference doc.
