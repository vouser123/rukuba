...
- Files touched: pt-rebuild/api/sync.js (deleted), pt-rebuild/public/js/offline.js (dead queue/sync methods removed, DB_VERSION bumped to 2)
- Validation: Verified all callers of removed methods — none exist in index.html or any other file. Active sync path (syncOfflineQueue in index.html) untouched. Caching methods (getCachedExercises etc.) still present and unchanged. PR created for preview deployment before merge.
- Follow-ups: Verify preview deployment: app loads, exercise logging works online and offline, history loads correctly. Merge only after PT session confirms no regressions.
