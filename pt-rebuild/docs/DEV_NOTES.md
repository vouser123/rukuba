...
## Closed Items
- [x] DN-029 | status:done | priority:P2 | risk:low | tags:[docs,cleanup] | file:pt-rebuild/docs/dev_notes.json,pt-rebuild/scripts/generate-dev-notes.mjs,pt-rebuild/AGENTS.md,pt-rebuild/docs/HISTORY.md,pt-rebuild/docs/dev_notes.schema.json | issue:Eliminate dated_entries and legacy_entries by consolidating closure narratives into closed_items with LE archive IDs for non-DN history. | resolved:2026-02-24
  - Problem: Development tracking was split between `closed_items` metadata, `dated_entries` narrative records, and a `legacy_entries` markdown blob. This duplication increased drift risk, raised token cost for agents, and required two close-loop writes for every completed DN item.
  - Root cause: The JSON format evolved incrementally: `closed_items` held completion status while narrative closure details remained in a separate `dated_entries` array, and pre-structure history remained embedded as raw markdown. The generator and workflow docs continued to depend on this dual-write model.
  - Change made: Migrated DN-linked `dated_entries` narratives onto matching `closed_items` items; converted non-DN `dated_entries` into `LE-###` done items with full narrative fields; moved `legacy_entries.markdown` to `docs/HISTORY.md` archive; removed both `dated_entries` and `legacy_entries` from canonical JSON; updated schema root requirements; updated generator validation/rendering to require six narrative fields on every closed item and render a single consolidated `Closed Items` section; updated AGENTS close-loop instructions to single-write `closed_items` with narrative fields.
  - Files touched: pt-rebuild/docs/dev_notes.json, pt-rebuild/docs/dev_notes.schema.json, pt-rebuild/scripts/generate-dev-notes.mjs, pt-rebuild/AGENTS.md, pt-rebuild/docs/HISTORY.md, pt-rebuild/docs/DEV_NOTES.md
  - Validation: Ran `npm run dev-notes:build` and `npm run dev-notes:check` after migration. Generator validations passed with no missing narrative fields on closed items and no DN overlap between open_items and closed_items.
  - Follow-ups: None.
...
- [x] DN-024 | status:done | priority:P3 | risk:low | tags:[docs] | file:pt-rebuild/docs/dev_notes.json,pt-rebuild/docs/DEV_NOTES.md | issue:Confirm follow-up dev-tracking for JSON-canonical migration review and record validation commands run. | resolved:2026-02-22
  ...
  - Follow-ups: None.
- [x] DN-003 | status:done | priority:P1 | risk:high | tags:[data-model,reliability,sync,api,supabase,migration] | file:pt-rebuild/api/sync.js,pt-rebuild/api/logs.js | issue:Prevent orphaned logs when sets insert fails (cleanup or transactional behavior). | resolved:2026-02-21
...
- [x] DN-004 | status:done | priority:P1 | risk:high | tags:[data-model,reliability,api] | file:pt-rebuild/api/logs.js | issue:Form data is matched to sets by array index instead of `set_number` in create/update flows. | resolved:2026-02-21
  ...
  - Validation: None.
  - Follow-ups: None.
- [x] LE-008 | status:done | priority:P3 | risk:low | tags:[cleanup] | file:pt-rebuild/docs/dev_notes.json | issue:Admin-role user blocked from patient app (programs, sync) | resolved:2026-02-21
  ...
  - Follow-ups: None.
- [x] LE-009 | status:done | priority:P3 | risk:low | tags:[notifications,email,api,ui,ios,data-model,migration] | file:pt-rebuild/docs/dev_notes.json | issue:Email notifications for clinical messages (Resend integration) | resolved:2026-02-21
...
- [x] DN-002 | status:done | priority:P0 | risk:medium | tags:[security,api,auth] | file:pt-rebuild/api/logs.js | issue:Add therapist-to-patient authorization check in `createActivityLog()` when `patient_id` differs from caller. | resolved:2026-02-20
  ...
  - Validation: None.
  - Follow-ups: None.
- [x] LE-006 | status:done | priority:P3 | risk:low | tags:[cleanup] | file:pt-rebuild/docs/dev_notes.json | issue:Exercise IDs: bug fix + data migration to proper UUID format | resolved:2026-02-20
...
- [x] LE-007 | status:done | priority:P3 | risk:low | tags:[migration,supabase,reliability] | file:pt-rebuild/docs/dev_notes.json | issue:Supabase migrations file corrupted by storage-internal trigger lines | resolved:2026-02-20
...
- [x] LE-004 | status:done | priority:P3 | risk:low | tags:[cleanup] | file:pt-rebuild/docs/dev_notes.json | issue:PT Editor archived exercise visibility toggles for Edit/Roles/Dosage selectors | resolved:2026-02-19
...
- [x] LE-005 | status:done | priority:P3 | risk:low | tags:[ui,data-model] | file:pt-rebuild/docs/dev_notes.json | issue:pt_editor date fields blank when editing existing exercises | resolved:2026-02-19
  ...
  - Follow-ups: None.
- [x] LE-001 | status:done | priority:P3 | risk:low | tags:[docs,api,reliability] | file:pt-rebuild/docs/dev_notes.json | issue:API docs aligned to post-consolidation route model | resolved:2026-02-18
...
- [x] LE-002 | status:done | priority:P3 | risk:low | tags:[performance,lcp,cleanup] | file:pt-rebuild/docs/dev_notes.json | issue:Removed redundant client-side form parameter backfill from index.html | resolved:2026-02-18
  ...
  - Follow-ups: None.
- [x] LE-003 | status:done | priority:P3 | risk:low | tags:[ui,reliability] | file:pt-rebuild/docs/dev_notes.json | issue:Timer audio cues aligned for duration/hold and >10s start/pause voice | resolved:2026-02-18
