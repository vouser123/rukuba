...
## Open Items
- [ ] DN-034 | status:in_progress | priority:P2 | risk:medium | tags:[migration,ui] | file:pt-rebuild/lib/pt-view.js, pt-rebuild/pages/pt-view.js, pt-rebuild/pages/pt-view.module.css, pt-rebuild/hooks/useMessages.js, pt-rebuild/components/MessagesModal.js, pt-rebuild/components/MessagesModal.module.css, pt-rebuild/components/ExerciseHistoryModal.js, pt-rebuild/components/ExerciseHistoryModal.module.css, pt-rebuild/components/NavMenu.js | issue:Phase 2 of Next.js Strangler Fig migration: pt_view.html → pages/pt-view.js at /pt-view. Read-only history dashboard with messages, patient notes, needs-attention alerts, summary stats, filters, and exercise history modal.
  - Context: All Phase 2 files created on nextjs branch. lib/pt-view.js contains all pure data functions. hooks/useMessages.js handles 30-second polling, send, archive, markRead with localStorage timestamps for badge counts. MessagesModal and ExerciseHistoryModal extracted to components/ (own state, own CSS modules). Local sub-components (PatientNotes, NeedsAttention, SummaryStats, FiltersPanel, HistoryList) stay in pt-view.js — page-specific only. NavMenu NAV_PAGES updated: pt_view href /pt_view.html → /pt-view. Next step: push to nextjs branch, verify on preview URL per standard verification checklist.
  - Constraints/Caveats: pt_view.html stays untouched until /pt-view is verified on preview. useOfflineQueue is NOT needed — pt_view is read-only (no log submission). All event handlers use onPointerUp (iOS Safari). localStorage keys: dismissedNotes, notesCollapsed, filtersExpanded, lastReadMessageTime, lastSentMessageTime.
- [ ] DN-033 | status:in_progress | priority:P2 | risk:medium | tags:[migration,ui] | file:pt-rebuild/package.json, pt-rebuild/next.config.mjs, pt-rebuild/pages/rehab.js, pt-rebuild/pages/rehab.module.css, pt-rebuild/pages/_app.js, pt-rebuild/lib/rehab-coverage.js, pt-rebuild/lib/supabase.js, pt-rebuild/hooks/useAuth.js, pt-rebuild/components/NavMenu.js, pt-rebuild/components/NavMenu.module.css, pt-rebuild/components/AuthForm.js, pt-rebuild/components/AuthForm.module.css, pt-rebuild/styles/globals.css | issue:Strangler Fig migration to Next.js (Pages Router). Existing static HTML pages remain untouched; new pages built in Next.js one at a time. Phase 1: scaffolding + shared architecture (lib/supabase.js, hooks/useAuth.js, components/NavMenu.js, components/AuthForm.js) + rehab_coverage migrated to pages/rehab.js.
  - Context: Phase 1 complete on nextjs branch. Shared architecture: lib/supabase.js, hooks/useAuth.js, components/NavMenu.js, components/AuthForm.js, lib/rehab-coverage.js, pages/rehab.js. Phase 1 verified on preview URL (auth, coverage matrix, NavMenu, navigation, no console errors). CSS modularized: styles/globals.css (CSS vars + resets, imported in _app.js), components/NavMenu.module.css (hamburger styles scoped to NavMenu), components/AuthForm.module.css (auth form styles scoped to AuthForm), pages/rehab.module.css (coverage/legend/summary styles scoped to rehab page). rehab.js no longer has any <link rel="stylesheet"> tags — all styles bundled via CSS Modules or _app.js global import. public/css/main.css, rehab-coverage.css, hamburger-menu.css unchanged — vanilla JS pages still use them. Next steps: Phase 2 (pt_view → /pt-view). Phase 3 and 4 broken into sub-phases in NEXTJS_MIGRATION.md.
